...
export function KanbanTab({
  ...
  const filterBlock = (block: DailyBlock) => {
    const m = materias.find(x => x.nome.toLowerCase() === block.subjectName.toLowerCase());
    if (m && !m.ativa) return false;
      ...
      <div className="flex-1 overflow-x-auto pb-4">
        <div className="flex gap-6 min-w-max h-full items-start">
          {[
            ...
            { id: 'concluido', label: 'Concluído', color: 'border-brand-green/50', bg: 'bg-brand-green/5' }
          ].map(column => {
            const columnBlocks = filteredBlocks.filter(b => b.status === column.id).sort((a, b) => a.order - b.order);
              ...
              <div 
                ...
                onDrop={async (e) => {
                  ...
                  if (draggedBlock && draggedBlock.status !== column.id) {
                    updateDailyBlock(draggedBlock.id, { status: column.id as any });
                  }
